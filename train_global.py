...
def inj_forward_text(
    ...
    if inj_embedding is not None:
        ...
        for (
            bsz,
            idx,
        ) in enumerate(inj_index):
            lll = new_inputs_embeds[
                bsz,
                idx + emb_length :,
            ].shape[0]
            new_inputs_embeds[
    ...
    pooled_output = last_hidden_state[
        torch.arange(
            ...
            device=r_input_ids.device,
        ),
        r_input_ids.to(torch.int).argmax(dim=-1),
    ]
