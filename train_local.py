...
from train_global import Mapper, inj_forward_text, th2image
...
def inj_forward_crossattention(
    ...
    if context is not None and "LOCAL" in context:
        ...
        attention_scores_local = torch.matmul(
            query_local,
            key_local.transpose(
                -1,
                -2,
        ...
        attention_probs_clone = attention_probs.clone().permute(
            (
                0,
                2,
                1,
        ...
        attention_probs_mask = attention_probs_clone[
            torch.arange(index_local.shape[0]),
            index_local,
        ]
...
def save_progress(
    ...
    if step is not None:
        torch.save(
            ...
            os.path.join(
                args.output_dir,
                f"local_mapper_{str(step).zfill(6)}.pt",
            ),
        )
    else:
        torch.save(
...
def validation(
    ...
    **kwargs,
    ...
    encoder_hidden_states = text_encoder(
        {
            ...
            "inj_embedding": inj_embedding,
            "inj_index": placeholder_idx,
        }
    )[0]

    image_obj = F.interpolate(
        example["pixel_values_obj"],
        (
            224,
            224,
        ),
    ...
    image_features_obj = image_encoder(
        image_obj,
        output_hidden_states=True,
    )
                ...
                encoder_hidden_states = text_encoder(
                    {
                        ...
                        "inj_embedding": inj_embedding,
                        "inj_index": placeholder_idx.detach(),
                    }
                )[0]
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    clip_images = [
                        ...
                        for img in batch["pixel_values_clip"]
                    ]
                    obj_images = [
                        th2image(img).resize(
                            (
                        ...
                        for img in batch["pixel_values_obj"]
                    ]
                    ...
                    obj_masks = [
                        ...
                        for img in batch["pixel_values_seg"]
                    ]
                    img_list = []
                    for (
                        syn,
                    ...
                    img_list = np.concatenate(
                        img_list,
                        axis=0,
                    )
                    Image.fromarray(img_list).save(
                        os.path.join(
                            args.output_dir,
                            f"{str(global_step).zfill(5)}.jpg",
                        )
                    )

            logs = {
                "loss_mle": loss_mle.detach().item(),
